  ...
  PROPERTIES.forEach(im=>{
    (im.interessados||[]).forEach((it,i)=>{
      ...
      arr.push({ id:'int_'+im.id+'_'+i, nome:it.nome, tipos:[it.tipo==='comprador'?'Comprador':'Locatário'],
        cpf:it.cpf||'—', email:it.email||'—', tel:it.telefone||it.contato||'—', cidade:'—', desde:'—', docs:[], obs:'Interessado no imóvel '+im.code, fichaId:it.fichaId||'', fichaTipo:it.fichaTipo||'' });
    });
  });
  PEOPLE = arr;
}
...
handleAction = function(a, el){
  ...
  if(a==='int-remover'){ if(!confirm('Remover este interessado do imóvel?'))return; const imovelId=el.dataset.imovel, idx=+el.dataset.idx; el.disabled=true; fnInteressado({imovelId, acao:'remover', index:idx}).then(async()=>{ toast('Interessado removido','trash-2'); await carregarDados(); const dr=$('#drawer'); if(state.currentProp===imovelId && dr && dr.classList.contains('show')) openProp(imovelId); }).catch(e=>{ toast(e.message||'Erro','alert-triangle','var(--danger)'); try{el.disabled=false;}catch(_e){} }); return; }
