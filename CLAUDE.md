...
- **Permissões granulares por pessoa** (concedidas no painel de Admin, nunca elevação em bloco): `marketing_gerenciar` (editar Marketing — NÃO herda de admin), `ti` (Suporte), `drives_fotografia`, `analise_locador`, `loc_beta` (**"Acesso de teste"** — dark launch da visão nova "Meus Negócios"; **religado em 2026-07-26**, controla quem VÊ a aba — ver "Dívida técnica intencional"). **`loc_gestao` foi REMOVIDA (2026-07-26)** — o papel dentro de "Meus Negócios" (gestor/broker vs administrativo vs corretor) agora vem SÓ da claim `locRole` (fonte única, via `locMeuPerfil`/`ehGestorAuth`); a checkbox saiu do Admin. (O campo `loc_gestao` ainda existe inerte no backend `setUserAccess`/`getPermissions` — grava `false`, ninguém lê; limpar no publish grande.) Perfil de Locação = claim `locRole` (gestor/administrativo/corretor) + `loc_financeiro`.
...
- **Gestão de Locações** (aba Locação): módulo completo — captação (ficha do locador vira imóvel na esteira) → análise do locatário + garantia → contrato → cobrança/repasse → vistorias → alertas → relatórios (com export CSV). **Versão enxuta (pedido do chefe, v1.0.90):** sub-apps = só **Painel** e **Imóveis**.
  ...
  - **Invariantes da caça-bugs 2026-07-28** (não reintroduzir):
    - **Identidade do interessado antes de agir por índice** (`carteiraInteressado` remover/status + `negocioGerar`):
      o índice vem do cliente e ENVELHECE se um interessado anterior for removido concorrentemente (splice
      desloca todo mundo). Por isso o cliente manda `esperaNome`+`esperaFichaId` e o servidor recusa com
      "recarregue" se `lista[i]` não bater (`_interessadoConfere`). ⚠️ Não voltar a agir só pelo índice — aprovava/
      removia a pessoa errada em silêncio. Aditivo: cliente velho sem as dicas passa como antes.
    - **Papel de Locação vem da claim `locRole` do token, NÃO de `locMeuPerfil`** (`hub-app.js`): ler direto do
      `getIdTokenResult` (bootstrap admin conta como gestor) — a Cloud Function `locMeuPerfil` num cold start/blip
      rebaixava gestor/administrativo pra visão de corretor até relogar. A chamada `locMeuPerfil` saiu do login.
    - **O timer de status (3 min) não re-renderiza o centro com o Broker embutido montado** (`hub-app.js`):
      `renderCentro` re-navegaria o Broker (`window.Broker.navigate`) e descartaria estado transitório (comentário
      sendo digitado). Guarda: `!(categoriaAtiva === 'locacoes' && brokerMontado)`.
    - **Materialização do imóvel no RECEBIMENTO da ficha** (`onFichaLocadorEnviadaAdmin`/`onFichaVendedorEnviadaAdmin`):
      o `MATERIALIZA` inclui **`aguardando_corretor`** além de `enviado_admin` — o imóvel nasce na Carteira assim que
      o cliente envia a ficha, **sem** o corretor aprovar/enviar ao broker (pedido do Nathan), mesmo com pendências.
      Idempotência por EXISTÊNCIA + diff estável por chave (`_jsonEstavel`, não `JSON.stringify` cru) pra não reverter
      edições que o corretor fez na Carteira num re-save que só mudou ordem de chaves.
  ...
  - **VISÃO NOVA DO BROKER** (2026-07-24, **só no staging**; pedido do Nathan, mockup `Broker (1).html` do Downloads): reskin completo da aba **"Meus Negócios"** (era "Locação" — a categoria `locacoes` foi **renomeada** pra "Meus Negócios") num **SPA dark de tela cheia** (sidebar própria + topbar), fiel ao HTML enviado.
    - **Como abre (2026-07-25, pedido do Nathan — SANFONA embutida, sem janela/tela nova)**: clicar em **"Meus Negócios"** na sidebar do Hub **expande uma sanfona** com os sub-itens do Broker (Dashboard/Pessoas/Imóveis/Negócios/Documentos/Relatórios… conforme o papel, **menos "Agenda"** que já existe no Hub) e o conteúdo do Broker aparece **DENTRO da área central do Hub** (`#bkRoot` é movido pra dentro de `.hub-main` e ganha a classe `bk-embedded`, que esconde a sidebar/topbar próprios do Broker). Vale pra **desktop E web** — o caminho de janela separada (`abrirMeusNegocios`/`broker.html`) foi **removido do fluxo** (o `broker.html`/IPC continuam no repo, inertes). No `hub-app.js`: `locSanfonaAberta`/`locBrokerView`/`brokerMontado`, `locPapel()`/`locSubItens()`, mount com `{embedded:true, view, onNavigate}` e troca de tela via `window.Broker.navigate(view)` (sem remontar; `getNav(role)` dá os itens; `onNavigate` sincroniza o destaque da sanfona quando a navegação nasce DENTRO do Broker). Pontes globais do Hub pro Broker: `window.hubAbrirCategoria(id)` (ex.: Agenda) e `window.hubAbrirApp(siteKey)` (ex.: ClickSign, checando permissão). No embutido, overlay/drawer/modal/toast do Broker são `position:absolute` (confinados à área central — não cobrem a sidebar do Hub). ⚠️ `FICHA_HOST` do broker-app resolve por hostname (staging→staging) — não voltar a cravar produção. Layout: `.hub-main--broker` (tira padding/scroll) + `.hub-layout.loc-embed` (esconde o painel direito → largura cheia); ao sair da categoria, `renderCentro` desmonta o Broker e restaura tudo. **As telas antigas (`renderCarteira`/`carregarPainel`/`renderTela03/04A/04B/05`) ficam de fallback** se `window.Broker` não existir. ⚠️ O conteúdo do Broker é **escuro** dentro do Hub claro (decisão aceita pelo Nathan).
    - **UI (pedido do Nathan)**: **Configurações** e **"sair da conta"** ficam **só no Hub**, NÃO no Broker (tirei o item Config do menu e o log-out do rodapé/topbar). O botão **"Voltar ao Hub"** fica no **topo-esquerdo** da sidebar (no lugar do antigo "RE/MAX SMART · Locação").
    - **Arquivos novos**: `broker-app.js` (módulo ES autossuficiente — reusa o app Firebase do hub via `getApps()`; expõe `window.Broker={mount,unmount}`), `broker.css` (CSS do mockup **escopado em `#bkRoot`** por um scoper — o mockup e o Hub compartilham classes `.topbar`/`.sidebar`/`.card`, então tem que ser namespaced; gerar de novo se mexer: `scratchpad/scope-css.js`), `broker.html` (página da JANELA do desktop — bootstrap espera auth + `window.Broker` e monta) e `vendor/lucide.min.js` (ícones — o `file://` do `.exe` não carrega o unpkg). Ligados no `index.html` (link + 2 scripts + a div `#bkRoot`), no `build-pwa.js` (COMPARTILHADOS + copia `vendor/`; `broker.html` NÃO vai pro PWA, que usa overlay) e no `package.json` `files` (`broker.css`, `vendor/**`; `*.html` já pega o `broker.html`). Lucide carrega global (~412KB) pra todos, mas só converte `[data-lucide]`, que o Hub não usa — sem impacto visual no resto. ⚠️ Esta versão do lucide (UMD) converte `<i data-lucide>` em `<svg>` **mantendo** o atributo — normal, ícone renderizado.
    - **TRÊS PAPÉIS no MESMO app** (`state.role`, roteado no `hub-app.js` branch `cat.locacoes` por `podeGestaoLoc`/`locRoleAtual`): **Broker/gestor** (Dashboard equipe, Pessoas, Imóveis, Negócios, Relatórios), **Corretor** (Dashboard + **Fichas Digitais** com link real `?corretor=<uid>&nome=`, Meus Clientes, Meus Imóveis, Meus Negócios, **Agenda**, **Documentos**, Minhas Comissões, Meu Perfil — só o dele; topbar com botão **"+ Novo"**), **Administrativo** (Dashboard operacional, **Fila de Trabalho**, Pessoas, Imóveis, Negócios, Documentos, **Clicksign**, **Google Drive**, **Agenda**, Relatórios Operacionais, Meu Perfil — vê tudo). O menu (`NAV_ROLE`), o dashboard (dispatch por papel) e os títulos (`hTitulo` → "Meus…") mudam por papel; o resto é reaproveitado (o backend JÁ filtra por `corretorUid`, então corretor só vê o dele sem código extra). **Meu Perfil** é real (`getMeuPerfil`/`salvarMeuPerfil` — telefone/CRECI/CPF).
    - **Fidelidade 1:1 aos mockups (2026-07-25)**: os 3 dashboards e telas foram reconstruídos **fiéis** aos HTMLs enviados (`Corretor (offline).html`/`Administrativo (offline).html`). **Corretor**: dashboard completo — 6 KPIs, faixa de atalhos, Fichas digitais, **Meu dia** (Próximas ações + Minhas pendências), **Meu pipeline**, **Agenda & recebimentos**, SMART IA; menu ganhou **Agenda** + **Documentos**; KPIs/ações/pendências/pipeline/comissões = **dado REAL** (dos negócios do corretor), Agenda-do-dia/SMART IA = demo. **Administrativo**: dashboard "Centro de operações" (6 cards + Resumo da fila + Tarefas críticas + SMART IA); **Fila de Trabalho** = kanban **operacional de 6 colunas do mockup** (`ADM_FILA`, DEMO — o fluxo Novas Fichas→Finalizado não existe no modelo de status real), + telas **Clicksign** (status vem do checklist real), **Google Drive** (pastas demo), **Agenda** (demo) e **Relatórios Operacionais** (demo). Todas as telas de demonstração carregam o selo **MODO DE TESTE** + aviso. Mockups: `Broker (1).html`, `Corretor (offline).html`, `Administrativo (offline).html` (bundles offline — extraí com `scratchpad/unbundle.js`). ⚠️ Gating (atualizado 2026-07-26): a aba `locacoes` **NÃO aparece mais pra todos** — voltou a ser **dark launch** por `loc_beta` ("Acesso de teste"); dentro dela o papel é roteado SÓ pela claim `locRole` (a permissão `loc_gestao` foi removida em 2026-07-26). Ver "Dívida técnica intencional".
    - **Design 1:1**: os renderizadores do mockup são portados quase verbatim e alimentados por **mappers** que convertem o retorno real das functions pro formato que o mockup espera (`mapImovel`/`mapNegocio`). **Dado REAL**: Imóveis (`locListarImoveis`), Negócios (`negocioListar`; detalhe/checklist/comentário via `negocioAtualizar` — escrita real), Relatórios (agrega no cliente), Pessoas (function NOVA `pessoasListar` — lista a coleção `pessoas` com regra de ouro, gestor vê tudo; + interessados dos imóveis). Corretores/fotos via `listarPessoas`+`listarFotosPerfil`.
    - **✅ SEM DADOS DE DEMONSTRAÇÃO (2026-07-25, pedido do Nathan "tire os dados falsos de todas as telas")**: removidos a constante `TEAM`, `teamCard`/`corretorDrawer`/`ADM_FILA`/`badgeTeste`/SMART IA e o kanban operacional demo. **Todas as telas usam dado real** dos negócios/imóveis (via `negocioListar`/`locListarImoveis`/`pessoasListar`): Broker (Performance da imobiliária com VGV/ticket reais, Funil por status, Carteira por finalidade, **Produção por corretor** agregada no cliente), Corretor (KPIs/Meu dia/pipeline/comissões reais), Administrativo (contadores, Resumo da fila, "Precisa de atenção" e Atividades recentes reais; **Fila** = kanban REAL por status; **Relatórios Operacionais** reais; **Clicksign** = status do checklist real). Onde ainda **não há fonte real** — **Documentos, Google Drive e Agenda** desta visão — a tela mostra estado honesto **"em breve"**, nunca número falso. Performance por corretor com Smart Score/conversão/ticket-trend individual (o antigo `TEAM`) fica pra quando existir agregação real no backend.
    - **Não implementado de propósito** (v1): criar negócio (nasce na Tela 03 antiga, regra do Manual), "Novo Cliente/Imóvel/Negócio" e o "+ Novo" (ficam no Cadastro/carteira antiga → toast "em breve"), upload/preview de doc, sync do Drive, Clicksign abrir/reenviar, export de relatório — tudo toast "em breve" (`emBreve`/`handleAction`). **Deploy no staging** (`firebase deploy --only hosting --project staging`); render dos 3 papéis conferido no arquivo servido (menus, +Novo, Meu dia, pipeline, fila kanban, relatórios op., zero "RE/MAX") — falta o **teste logado** (corretor/adm/gestor) pra ver dado real e as escritas de checklist/comentário. **Nada em produção.**
  ...
  - **Gerar Contrato de representação — SÓ VENDA** (2026-07-24, **só no staging**; pedido do Nathan): botão **"📄 Gerar Contrato"** no cabeçalho da Tela 03 (Detalhes do Imóvel), visível **só** quando `finalidade` é `venda` ou `venda_locacao`. Preenche o **PDF-modelo editável da RE/MAX** (`functions/assets/contrato-representacao-pf.pdf`, "PF - 04 Contratantes") e devolve pronto pra baixar; o corretor sobe no ClickSign (não há integração por API — ClickSign no Hub é só autologin). Function `gerarContratoVenda({imovelId})` (posse: dono ou gestor; recusa não-venda) usa **`pdf-lib`** pra preencher os campos AcroForm.
  - **Tela Documentos — real, com upload por negócio + sanfona de clientes** (2026-07-28, **PROD + staging + `.exe` v1.0.114**; pedido do Nathan a partir do mockup): a tela **Documentos** do Broker deixou de ser placeholder "em breve" e virou dado REAL, em duas partes:
    1. **Documentos do negócio** — agrupados em **Contratos / Propostas / Documentação do cliente / Outros** (as seções do mockup). **Gestor e administrativo SOBEM e REMOVEM** (botão "Enviar documento" → escolhe negócio ativo + categoria + arquivo); **corretor só VÊ e BAIXA** os dos seus negócios. Encerrado (concluído/cancelado) não aceita novo upload.
    2. **Sanfona "Documentos dos clientes"** (`<details>`) — lista as fichas de locatário/comprador (pf/pj/fiador/proposta/fiança) que têm anexos: **nome + imóvel vinculado** (se é interessado de algum, via `imoveis.interessados[].fichaId`) **ou "Não vinculado"** + expandir pra ver/baixar cada documento (URL real da própria ficha).
    - **Functions novas**: `negocioAnexarDoc` (gestor/adm; arquivo em **base64** → Admin SDK grava no Storage em `negocios/<negocioId>/<uuid>_<nome>` com **token de download** próprio, igual às fichas; regra de ouro; **PDF ou imagem raster — SVG fica de fora** de propósito porque é XML e poderia carregar script na URL pública; ≤20MB, cap 100/negócio, transacional + timeline + audit; se a transação falha, **apaga o arquivo órfão** do Storage), `negocioRemoverDoc` (gestor/adm; tira do array + do Storage) e `documentosClientes` (role-scoped/regra de ouro, `limit(500)` por tipo pra não truncar em silêncio; `_docCliLabel` humaniza a chave do anexo). `negocioListar`/`_negocioSerializar` passaram a serializar `documentos[]`. Bucket via `PROJECT_BUCKET` (por ambiente).
    - **Download**: a URL é `firebasestorage…?alt=media&token=…` (o token é a credencial, como fichas/portais). **No `.exe`** o `main.js` (setWindowOpenHandler) roteia `firebasestorage.googleapis.com` pra `shell.openExternal` → baixa no navegador do SO; na web abre aba normal.
    - No menu do Broker o item **Documentos** foi adicionado ao papel **gestor** (corretor/administrativo já tinham).
    - **O que a gente injeta**: vendedores 1–4 (nome/RG/CPF/endereço/nacionalidade/estado civil/email/tel1/tel2, da **ficha do vendedor** vinculada — PII fica na `fichas/{fichaId}.dados`, prefixos `''`/`loc2_`/`loc3_`/`loc4_`), o imóvel (tipo/endereço/complemento/bairro/município/estado/classificação fiscal=contribuinteIptu/valor/observações) e o **corretor DONO do imóvel** (nome do imóvel; **CRECI/CPF do perfil dele** — `user_profiles.creci`/`cpf`, campos novos em Configurações). Nacionalidade default `Brasileiro(a)`.
    - **Já vem pronto no modelo** (não mexer): dados da REMAX (Smart Imobiliária LTDA, CNPJ 32.184.671/0001-46, sede SP), prazo 180 dias, comissão 6%, foro São Paulo, todo o texto jurídico. **Em branco de propósito**: matrícula/cartório (não capturamos), testemunhas e assinaturas (assinam depois).
...
- **Staging (projeto `remax-smart-hub-staging`, plano Blaze) — NO AR** em
  ...
  `admin@teste.local` (admin, senha `teste1234`) e `teste@staggin.com.br` (admin + gestor de Locação via
  claim `locRole:gestor`, senha `12345678`). **Contas das visões novas** (2026-07-24):
  `corretor@staggin.com.br` (claim `locRole:corretor`) e `adm@staggin.com.br`
  (claim `locRole:administrativo`), ambas senha `12345678` — pra testar as visões Corretor e Administrativo.
  Todas com `loc_beta:true` (o dark launch libera a aba "Meus Negócios"); o papel vem só da claim `locRole`.
  Semeadas por **`node scripts/seed-staging.js`**
