...
- **Permissões granulares por pessoa** (concedidas no painel de Admin, nunca elevação em bloco): `marketing_gerenciar` (editar Marketing — NÃO herda de admin), `ti` (Suporte), `drives_fotografia`, `analise_locador`, `loc_gestao` (decide o **papel** dentro de "Meus Negócios" — gestor/broker; NÃO herda de admin), `loc_beta` (**"Acesso de teste"** — dark launch da visão nova "Meus Negócios"; **religado em 2026-07-26**, controla quem VÊ a aba — ver "Dívida técnica intencional"). Perfil de Locação = claim `locRole` (gestor/administrativo/corretor) + `loc_financeiro`.
...
- **Staging (projeto `remax-smart-hub-staging`, plano Blaze) — NO AR** em
  ...
  senha `12345678`, aba Locação ligada via `user_access.loc_gestao`). **Contas das visões novas** (2026-07-24):
  `corretor@staggin.com.br` (claim `locRole:corretor`, sem loc_gestao) e `adm@staggin.com.br`
...
- **⚠️ `betaLocacoes`/`locacoesPublicado` NÃO são mais inertes (religados em 2026-07-26).** A aba
  ...
  **só quem tem "Acesso de teste" (`loc_beta`) vê**, e o painel de **Lançamento** volta a valer como o
  ...
  nova só nos testadores até o publish grande. `loc_gestao` continua sendo o que decide o **papel** dentro
  da aba (gestor/broker), não mais a visibilidade dela.
