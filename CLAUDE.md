...
**bloco de notas** e uma **área admin**. Versão publicada atual: **1.0.94** (auto-update via GitHub Releases).
...
- **Bug Fix Bot** (autônomo, "propõe → você dispõe"): ✅ FUNCIONANDO no disparo manual (2026-07-13; PR #1 aberto de ponta a ponta). Secrets no GitHub OK (`CLAUDE_CODE_OAUTH_TOKEN` da assinatura + checkbox "Allow GitHub Actions to create PRs"). FALTA: fase 2 (caça-bugs diário 00h + email-resumo pro Nathan + página web de aprovação estilo portais, com token) e o caminho automático via Firebase (`BOT_GH_TOKEN` + deploy de `botCorrigirBug`/`onErroParaBot`). O Claude roda no GitHub Actions, investiga um bug num branch isolado, roda os checks da CI e **abre um Pull Request** pra você revisar. **Nunca** faz merge, sobe versão ou publica — isso segue 100% manual.
