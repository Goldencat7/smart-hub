...
- **Gestão de Locações** (aba Locação): módulo completo — captação (ficha do locador vira imóvel na esteira) → análise do locatário + garantia → contrato → cobrança/repasse → vistorias → alertas → relatórios (com export CSV). **Versão enxuta (pedido do chefe, v1.0.90):** sub-apps = só **Painel** e **Imóveis**.
  ...
  - **VISÃO NOVA DO BROKER** (2026-07-24, **só no staging**; pedido do Nathan, mockup `Broker (1).html` do Downloads): reskin completo da aba **"Meus Negócios"** (era "Locação" — a categoria `locacoes` foi **renomeada** pra "Meus Negócios") num **SPA dark de tela cheia** (sidebar própria + topbar), fiel ao HTML enviado.
    - **Como abre**: **DESKTOP (.exe)** → **janela PRÓPRIA** (como abrir um app): `hub-app.js` detecta desktop (`hubApi.autologin===true`) e chama `hubApi.abrirMeusNegocios()` → IPC `abrir-meus-negocios` no `main.js` abre `broker.html` numa `BrowserWindow` singleton (foca se já aberta); a sessão Firebase é a MESMA do Hub (janelas `file://` dividem o IndexedDB → já entra logado). O "Voltar ao Hub" fecha a janela (`hubApi.fecharJanela` → IPC `fechar-janela`), e o Hub principal volta pra 1ª categoria não-locação. **WEB/PWA** → **overlay** (`#bkRoot`) por cima do Hub (não tem janela separada no navegador); "Voltar ao Hub" desmonta e troca de categoria. **As telas antigas (`renderCarteira`/`carregarPainel`/`renderTela03/04A/04B/05`) ficam intactas como fallback** se `window.Broker` não existir.
    ...
    - **TRÊS PAPÉIS no MESMO app** (`state.role`, roteado no `hub-app.js` branch `cat.locacoes` por `podeGestaoLoc`/`locRoleAtual`): **Broker/gestor** (Dashboard equipe, Pessoas, Imóveis, Negócios, Relatórios), **Corretor** (Dashboard + **Fichas Digitais** com link real `?corretor=<uid>&nome=`, Meus Clientes, Meus Imóveis, Meus Negócios, Minhas Comissões, Meu Perfil — só o dele), **Administrativo** (Dashboard operacional, **Fila de Trabalho**=kanban real de negócios por status, Pessoas, Imóveis, Negócios, Documentos, Relatórios Operacionais, Meu Perfil — vê tudo). O menu (`NAV_ROLE`), o dashboard (dispatch por papel) e os títulos (`hTitulo` → "Meus…") mudam por papel; o resto é reaproveitado (o backend JÁ filtra por `corretorUid`, então corretor só vê o dele sem código extra). **Meu Perfil** é real (`getMeuPerfil`/`salvarMeuPerfil` — telefone/CRECI/CPF). MODO DE TESTE: SMART IA (corretor), Documentos (placeholder). Mockups: `Broker (1).html`, `Corretor (offline).html`, `Administrativo (offline).html` (bundles offline — extraí com `scratchpad/unbundle.js`). ⚠️ Gating: a aba `locacoes` agora aparece pra TODOS (roteia por papel) — **decidir gating de produção antes de publicar**.
    ...
    - **⚠️ MODO DE TESTE (dados de DEMONSTRAÇÃO, marcados na tela com selo "MODO DE TESTE")**: TODO o dashboard de **Performance da equipe** (Smart Score, ranking, captações/conversão/VGV/ticket, produção mensal, drawer do corretor) e o bloco **SMART IA** usam a constante `TEAM` embutida — **não há fonte real** dessas métricas ainda. O Nathan pediu pra deixar a IA "em modo de teste" até entender/definir. O "Centro de operações" (contadores) e a carteira por finalidade JÁ são reais. Quando for ligar de verdade, criar a agregação por corretor no backend e trocar `TEAM`/`ia`/`funil` pelos dados reais.
    - **Não implementado de propósito** (v1): criar negócio (nasce na Tela 03 antiga, regra do Manual), "Novo imóvel"/"Nova pessoa" (ficam no Cadastro/carteira antiga), export de relatório (toast "em breve"). **Testado no staging** sem login (shell+6 telas renderizam, ícones/CSS, zero erro de console) — falta o teste logado como gestor pra ver dado real e as escritas de checklist/comentário. **Nada em produção.**
